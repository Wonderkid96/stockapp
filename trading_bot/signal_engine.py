...
def get_indicator_data(db: Session, symbol: str, days: int = 10) -> pd.DataFrame:
    ...
    ).order_by(Indicator.timestamp.desc()).limit(days)
    result = query.all()
    if not result:
...
def detect_ma_crossover(df: pd.DataFrame) -> list:
    ...
    if 'sma_20' in df.columns and 'ema_50' in df.columns:
        prev_row = df.iloc[-2]
        curr_row = df.iloc[-1]
        if prev_row['sma_20'] <= prev_row['ema_50'] and curr_row['sma_20'] > curr_row['ema_50']:
            signals.append({
                'signal_type': 'BUY',
        ...
        elif prev_row['sma_20'] >= prev_row['ema_50'] and curr_row['sma_20'] < curr_row['ema_50']:
            signals.append({
                'signal_type': 'SELL',
...
def detect_signals(db: Session, symbols: list):
    ...
    for symbol in symbols:
        ...
        if indicator_data.empty:
            logger.warning(f"No indicator data available for {symbol}, skipping signal detection")
            continue
        latest_timestamp = indicator_data.index[-1]
        existing = db.query(Signal).filter(
            Signal.symbol == symbol,
            Signal.timestamp == latest_timestamp
        ).first()
        if existing:
            logger.info(f"Signals already exist for {symbol} at {latest_timestamp}, skipping")
            continue
        all_signals = []
        ...
        if all_signals:
            ...
            logger.info(f"Detected {count} new signals for {symbol} at {latest_timestamp}")
        else:
